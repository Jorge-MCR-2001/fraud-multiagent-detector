Detector de Fraudes Multi Agentico - Nivel 04

-> Objetivo:
    Implementar infraestructura Cloud

-> Tecnologia:
    .env.example                      -> Azure Key Vault
    policy_embeddings.npy             -> Azure AI Search vector index
    audit_trail.jsonl                 -> Cosmos DB
    hitl_queue.jsonl                  -> Cosmos DB
    agent_events.jsonl                -> Application Insights / Cosmos DB
    FastAPI local                     -> Azure Container Apps
    OpenAI local config               -> Azure OpenAI / Key Vault
